...
Arrays of unknown size (C99-complaint variable length structs specified by `[]` or `[0]`) are not supported.
...
its functions can only accept a function signature with a statically-known fixed signature.
