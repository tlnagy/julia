...
static void typeassert_input(const jl_cgval_t &jvinfo, jl_value_t *jlto, int argn, bool addressOf, jl_codectx_t *ctx)
{
    if (jvinfo.typ != jlto && jlto != (jl_value_t*)jl_any_type) {
...
static Value *julia_to_address(Type *to, jl_value_t *jlto, const jl_cgval_t &jvinfo,
                               int argn, jl_codectx_t *ctx, bool *needStackRestore)
{
    ...
    if (jlto == (jl_value_t*)jl_voidpointer_type) {
        ...
        ety = jl_tparam0(jlto);
        typeassert_input(jvinfo, ety, argn, true, ctx);
    }
    ...
    Type *slottype = julia_struct_to_llvm(jvinfo.typ, NULL);
...
static Value *julia_to_native(Type *to, bool toboxed, jl_value_t *jlto,
                              ...
                              bool *needStackRestore)
{
    ...
    typeassert_input(jvinfo, jlto, argn, false, ctx);
    if (!byRef)
...
static jl_cgval_t emit_llvmcall(jl_value_t **args, size_t nargs, jl_codectx_t *ctx)
{
    ...
    for (size_t i = 0; i < nargt; ++i) {
        ...
        Value *v = julia_to_native(t, toboxed, tti, arg, false, i, ctx, NULL);
...
{
    if (!static_rt) {
        assert(!isboxed && ctx->spvals_ptr && jl_is_datatype(rt));
        // box if concrete type was not statically known
        Value *args[3];
        args[0] = literal_pointer_val(rt);
        args[1] = literal_pointer_val((jl_value_t*)ctx->linfo->def->tvars);
        ...
        Value *runtime_dt = builder.CreateCall(prepare_call(jlapplytype_func), ArrayRef<Value*>(args));
...
public:
    ...
    jl_svec_t *tvars; // UnionAll environment for `at` and `rt`
    size_t nargs; // number of actual arguments (can be different from the size of at when varargs)
    size_t isVa;

    function_sig_t(Type *lrt, jl_value_t *rt, bool retboxed, jl_svec_t *at, jl_svec_t *tvars, size_t nargs, size_t isVa, CallingConv::ID cc, bool llvmcall)
      : fargt_vasig(NULL), lrt(lrt), retboxed(retboxed),
        prt(NULL), sret(0), cc(cc), llvmcall(llvmcall),
        functype(NULL), at(at), rt(rt), tvars(tvars),
    ...
    jl_cgval_t emit_a_ccall(const native_sym_arg_t &symarg,
            ...
            jl_value_t *rt_expr, bool static_rt,
            jl_codectx_t *ctx);
...
static const std::string verify_ccall_sig(size_t nargs, jl_value_t *&rt, jl_value_t *at,
                                          jl_svec_t *tvars, jl_svec_t *sparam_vals, const char *funcName,
                                          size_t &nargt, bool &isVa, Type *&lrt, bool &retboxed, bool &static_rt)
{
    ...
    lrt = julia_struct_to_llvm(rt, &retboxed);
    if (lrt == NULL)
    ...
    if (tvars == jl_emptysvec) {
        ...
        static_rt = retboxed || !jl_has_typevars(rt);
        unsigned ntvars = jl_is_svec(tvars) ? jl_svec_len(tvars) : 1;
        if (!static_rt && ntvars == jl_svec_len(sparam_vals)) {
            rt = jl_instantiate_type_in_env(rt, (jl_value_t*)tvars, jl_svec_data(sparam_vals));
            static_rt = true;
        }
    }
...
static jl_cgval_t emit_ccall(jl_value_t **args, size_t nargs, jl_codectx_t *ctx)
{
    ...
    jl_svec_t *tvars = ctx->linfo->def ? ctx->linfo->def->tvars : jl_emptysvec;
    if (jl_is_abstract_ref_type(rt)) {
        ...
        if (ref == (jl_value_t*)jl_any_type) {
            ...
            if (tvars != jl_emptysvec) {
                ...
                jl_value_t **tv;
                if (jl_is_svec(tvars)) {
                    tv = jl_svec_data(tvars);
                    ntv = jl_svec_len(tvars);
                }
                else {
                    tv = (jl_value_t**)&tvars;
                    ntv = 1;
    ...
    function_sig_t sig(lrt, rt, retboxed, (jl_svec_t*)at, tvars, (nargs - 3) / 2, isVa, cc, llvmcall);
        ...
        jl_codectx_t *ctx)
{
    ...
    //jl_svec_t *tvars = ctx->linfo->def ? ctx->linfo->def->tvars : jl_emptysvec;
    //unsigned ntvars = jl_is_svec(tvars) ? jl_svec_len(tvars) : 1;
    jl_value_t *jargty = NULL; // Julia type of the current parameter
    JL_GC_PUSH1(&jargty);
    ...
    for (size_t ai = 0; ai < nargs; ai++) {
        ...
        if (!toboxed && tvars != jl_emptysvec && jl_has_typevars(jargty) && ntvars == jl_svec_len(ctx->linfo->sparam_vals))
            jargty_in_env = jl_instantiate_type_in_env(jargty_in_env, (jl_value_t*)tvars, jl_svec_data(ctx->linfo->sparam_vals));
        ...
        if (!addressOf.at(ai)) {
            ...
            v = julia_to_native(largty, toboxed, jargty_in_env, arg, byRef,
            ...
            assert(!toboxed && !byRef);
            v = julia_to_address(largty, jargty_in_env, arg,
                                 ai + 1, ctx, &needStackRestore);
        }
        if (isa<UndefValue>(v)) {
            JL_GC_POP();
    ...
    return mark_or_box_ccall_result(result, jlretboxed, rt_expr, rt, static_rt, ctx);
}
